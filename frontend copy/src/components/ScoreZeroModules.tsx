  ...
  return (
    ...
    >
      ...
      <div className="content-grid">
        <img
          src={project.image}
          className="main-image w-full h-full object-cover rounded-2xl md:rounded-3xl border border-white/20"
          alt={project.title}
...
const ScoreZeroModules = () => {
  ...
  useLenis(({ scroll }) => {
    ...
    if (thread && threadLen > 0 && isMobile) {
      ...
      const animateText = (ref: React.RefObject<SVGTextElement | null>, targetP: number) => {
  ...
  return (
    <section id="work" className="min-h-screen bg-black text-white font-sans relative">
      <div id="modules" className="absolute -top-20" />
          ...
          <div className="marquee-selected-works__track">
            {[0, 1, 2, 3].map((blockIndex) => (
              <div key={blockIndex} className="marquee-selected-works__segment" aria-hidden={blockIndex > 0 ? "true" : undefined}>
                <span className="marquee-selected-works__text">ScoreZero Modules</span>
      ...
      <div ref={kineticWheelRef} className="kinetic-wheel pointer-events-none" style={{
        ...
        {isMobile ? (
          <svg viewBox="0 0 1500 2000" className="w-full h-full" style={{ overflow: 'visible' }}>
            <defs>
              <linearGradient id="line-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
                ...
                <stop offset="15%" stopColor="rgba(0,210,255,0.7)" />
                <stop offset="85%" stopColor="rgba(0,210,255,0.7)" />
            ...
            <g ref={figureGroupRef}>
              <text ref={textAnalyzeRef} x="750" y="150" fill="#00D2FF" style={{ fontFamily: 'sans-serif', fontWeight: 800, fontSize: '100px', opacity: 0.3 }} textAnchor="middle" dy=".3em">PARSE</text>
              <circle cx="750" cy="250" r="15" fill="#00D2FF" filter="url(#glow)" />

              <text ref={textDesignRef} x="250" y="700" fill="#00D2FF" style={{ fontFamily: 'sans-serif', fontWeight: 800, fontSize: '100px', opacity: 0.3 }} textAnchor="middle" dy=".3em">DETECT</text>
              <circle cx="250" cy="800" r="15" fill="#00D2FF" filter="url(#glow)" />

              <text ref={textBuildRef} x="1250" y="1250" fill="#00D2FF" style={{ fontFamily: 'sans-serif', fontWeight: 800, fontSize: '100px', opacity: 0.3 }} textAnchor="middle" dy=".3em">SCORE</text>
              <circle cx="1250" cy="1350" r="15" fill="#00D2FF" filter="url(#glow)" />

              <text ref={textDeliverRef} x="750" y="1800" fill="#00D2FF" style={{ fontFamily: 'sans-serif', fontWeight: 800, fontSize: '100px', opacity: 0.3 }} textAnchor="middle" dy=".3em">DISPATCH</text>
              <circle cx="750" cy="1900" r="20" fill="#00D2FF" filter="url(#glow)" />
            </g>
          </svg>
        ) : (
          <svg viewBox="0 0 3000 1500" className="w-full h-auto" style={{ overflow: 'visible' }}>
            ...
            {[
              { text: 'PARSE', offset: '15%' }, { text: '●', offset: '27%' },
              { text: 'DETECT', offset: '38%' }, { text: '●', offset: '50%' },
              { text: 'SCORE', offset: '62%' }, { text: '●', offset: '73%' },
              { text: 'DISPATCH', offset: '85%' },
            ].map((item, i) => (
              <text key={i} fill="#00D2FF" style={{ fontFamily: 'sans-serif', fontWeight: 800, fontSize: item.text === '●' ? '50px' : '100px', textTransform: 'uppercase' }} dy={item.text === '●' ? '-18' : '0'}>
...
export default ScoreZeroModules;
